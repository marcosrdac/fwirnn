...
from utils.plotutils import plot_seismograms
...
if __name__ == '__main__':
    ...
    array_desc = dict(
        ...
        geometry=f'{0*dx}-{0*dx}-0-{0*dx}-{0*dx}',
        rr=dx,
        # ss=(nx // 20) * dx,
        # ss=(nx // 20) * dx,
        # ss=12 * dx,
    ...
    sp_order = 8
    # seismic source
    freq_max = calc_freq_max(v.min(), dz, dx, sp_order=sp_order)
    freq = .5 * freq_max
    signal = rickerwave(freq, dt)
    srcsgns = make_srcsgns(signal)
